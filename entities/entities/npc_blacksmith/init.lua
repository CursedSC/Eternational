...
function ENT:Initialize()
	...
	self:SetNWString("Name", "Михаил")
	self:SetNWString("Job", "Кузнец")
end
...
function ENT:GetDialogue(ply)
	for k, v in pairs(ply.Quests["Complete"]) do
		if v.name == "Тайны Зельевара #1" or v.name == "Жаркое дело #3" then
	...
	local dialogue = "npc_blacksmith_start"
	...
	for k, v in pairs(ply.Quests["Main"]) do
		if v.name == "Жаркое дело #1" then
			...
			if allcomplete then
				dialogue = "npc_blacksmith_secondstage"
			else
				dialogue = "npc_blacksmith_wait"
			end
			break
		end
		if v.name == "Жаркое дело #2" then
			dialogue = v.tasks[1].iscompleted and "npc_blacksmith_thirdstage" or "npc_blacksmith_test"
		end
		if v.name == "Жаркое дело #3" then
			...
			if allcomplete then
				dialogue = "npc_blacksmith_end"
			else
				dialogue = "npc_blacksmith_waitsword"
			end
			break
		end
	end
